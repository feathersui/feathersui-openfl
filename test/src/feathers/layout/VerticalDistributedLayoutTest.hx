...
@:keep
class VerticalDistributedLayoutTest extends Test {
	...
	private var _layout:VerticalDistributedLayout;
	...
	public function setup():Void {
		this._layout = new VerticalDistributedLayout();
	...
	public function testTwoItemsWithNullMeasurements():Void {
		...
		Assert.equals(2.0 * Math.max(CHILD1_HEIGHT, CHILD2_HEIGHT), result.viewPortHeight);
		Assert.equals(Math.max(CHILD1_WIDTH, CHILD2_WIDTH), result.contentWidth);
		Assert.equals(2.0 * Math.max(CHILD1_HEIGHT, CHILD2_HEIGHT), result.contentHeight);
		Assert.equals(0.0, result.contentX);
		Assert.equals(0.0, result.contentY);
	}
	...
	public function testTwoItemsWithGapAndNullMeasurements():Void {
		...
		var result = this._layout.layout([this._child1, this._child2], this._measurements);
		Assert.equals(Math.max(CHILD1_WIDTH, CHILD2_WIDTH), result.viewPortWidth);
		Assert.equals(2.0 * Math.max(CHILD1_HEIGHT, CHILD2_HEIGHT) + GAP, result.viewPortHeight);
		Assert.equals(Math.max(CHILD1_WIDTH, CHILD2_WIDTH), result.contentWidth);
		Assert.equals(2.0 * Math.max(CHILD1_HEIGHT, CHILD2_HEIGHT) + GAP, result.contentHeight);
		Assert.equals(0.0, result.contentX);
		Assert.equals(0.0, result.contentY);
	}
	...
	public function testTwoItemsWithPaddingAndNullMeasurements():Void {
		...
		Assert.equals(PADDING_TOP + PADDING_BOTTOM + 2.0 * Math.max(CHILD1_HEIGHT, CHILD2_HEIGHT), result.viewPortHeight);
		Assert.equals(PADDING_LEFT + PADDING_RIGHT + Math.max(CHILD1_WIDTH, CHILD2_WIDTH), result.contentWidth);
		Assert.equals(PADDING_TOP + PADDING_BOTTOM + 2.0 * Math.max(CHILD1_HEIGHT, CHILD2_HEIGHT), result.contentHeight);
		Assert.equals(0.0, result.contentX);
		Assert.equals(0.0, result.contentY);
	}
	...
	public function testTwoItemsWithPaddingGapAndNullMeasurements():Void {
		...
		var result = this._layout.layout([this._child1, this._child2], this._measurements);
		Assert.equals(PADDING_LEFT + PADDING_RIGHT + Math.max(CHILD1_WIDTH, CHILD2_WIDTH), result.viewPortWidth);
		Assert.equals(PADDING_TOP + PADDING_BOTTOM + 2.0 * Math.max(CHILD1_HEIGHT, CHILD2_HEIGHT) + GAP, result.viewPortHeight);
		Assert.equals(PADDING_LEFT + PADDING_RIGHT + Math.max(CHILD1_WIDTH, CHILD2_WIDTH), result.contentWidth);
		Assert.equals(PADDING_TOP + PADDING_BOTTOM + 2.0 * Math.max(CHILD1_HEIGHT, CHILD2_HEIGHT) + GAP, result.contentHeight);
		Assert.equals(0.0, result.contentX);
		Assert.equals(0.0, result.contentY);
	}

	public function testTwoItemsWithExplicitHeight():Void {
		var viewPortHeight = 640.0;
		this._measurements.height = viewPortHeight;
		this._layout.layout([this._child1, this._child2], this._measurements);
		var itemHeight = viewPortHeight / 2.0;
		Assert.equals(itemHeight, this._child1.height);
		Assert.equals(itemHeight, this._child2.height);
	}

	public function testTwoItemsWithExplicitHeightAndLargerItemHeights():Void {
		var viewPortHeight = 640.0;
		this._child1.height = 720.0;
		this._child2.height = 800.0;
		this._measurements.height = viewPortHeight;
		this._layout.layout([this._child1, this._child2], this._measurements);
		var itemHeight = viewPortHeight / 2.0;
		Assert.equals(itemHeight, this._child1.height);
	...
	public function testThreeItemsWithIncludeInLayoutFalse():Void {
		...
		var itemHeight = viewPortHeight / (items.length - 1);
		Assert.equals(itemHeight, this._control1.height);
		Assert.equals(0.0, this._control2.height);
		Assert.equals(itemHeight, this._control3.height);
	}

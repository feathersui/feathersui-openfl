...
@:keep
class MenuBarMeasurementTest extends Test {
	...
	public function setup():Void {
		...
		this._menuBar.themeEnabled = false;
		this._menuBar.layout = new HorizontalLayout();
		this._menuBar.dataProvider = new ArrayHierarchicalCollection([]);
		this._menuBar.itemToText = (item:Dynamic) -> null;
		this._menuBar.itemRendererRecycler = DisplayObjectRecycler.withFunction(() -> {
			var itemRenderer = new ItemRenderer();
			itemRenderer.width = SMALL_ITEM_WIDTH;
			itemRenderer.height = SMALL_ITEM_HEIGHT;
			return itemRenderer;
		});
		Lib.current.addChild(this._menuBar);
	}

	public function teardown():Void {
		if (this._menuBar.parent != null) {
			this._menuBar.parent.removeChild(this._menuBar);
		}
		this._menuBar = null;
	...
	private function addSimpleBackgroundSkin(?width:Float, ?height:Float):Void {
		...
		this._menuBar.backgroundSkin = backgroundSkin;
	}
	...
	private function addSingleChild(?width:Float, ?height:Float):Void {
		...
		this._menuBar.dataProvider.addAt({}, [0]);
		this._menuBar.itemRendererRecycler = DisplayObjectRecycler.withFunction(() -> {
			var itemRenderer = new ItemRenderer();
			itemRenderer.width = width;
			itemRenderer.height = height;
			return itemRenderer;
		});
	}
	...
	public function testAutoSizeWithNoChildren():Void {
		this._menuBar.validateNow();
		Assert.equals(0.0, this._menuBar.width);
		Assert.equals(0.0, this._menuBar.height);
		Assert.equals(0.0, this._menuBar.minWidth);
		Assert.equals(0.0, this._menuBar.minHeight);
		Assert.equals(Math.POSITIVE_INFINITY, this._menuBar.maxWidth);
		Assert.equals(Math.POSITIVE_INFINITY, this._menuBar.maxHeight);
	}

	public function testAutoSizeWithSimpleBackgroundSkinAndNoChildren():Void {
		this.addSimpleBackgroundSkin();
		this._menuBar.validateNow();
		Assert.equals(SMALL_BACKGROUND_WIDTH, this._menuBar.width);
		Assert.equals(SMALL_BACKGROUND_HEIGHT, this._menuBar.height);
		Assert.equals(SMALL_BACKGROUND_WIDTH, this._menuBar.minWidth);
		Assert.equals(SMALL_BACKGROUND_HEIGHT, this._menuBar.minHeight);
		Assert.equals(Math.POSITIVE_INFINITY, this._menuBar.maxWidth);
		Assert.equals(Math.POSITIVE_INFINITY, this._menuBar.maxHeight);
	}
	...
	public function testAutoSizeWithOneChildAndNoBackground():Void {
		this.addSingleChild();
		this._menuBar.validateNow();
		Assert.equals(SMALL_ITEM_WIDTH, this._menuBar.width);
		Assert.equals(SMALL_ITEM_HEIGHT, this._menuBar.height);
		Assert.equals(SMALL_ITEM_WIDTH, this._menuBar.minWidth);
		Assert.equals(SMALL_ITEM_HEIGHT, this._menuBar.minHeight);
		Assert.equals(Math.POSITIVE_INFINITY, this._menuBar.maxWidth);
		Assert.equals(Math.POSITIVE_INFINITY, this._menuBar.maxHeight);
	}
	...
	public function testAutoSizeWithChildAndPadding():Void {
		...
		this._menuBar.layout = layout;
		this._menuBar.validateNow();
		Assert.equals(PADDING_LEFT + SMALL_ITEM_WIDTH, this._menuBar.width);
		Assert.equals(PADDING_TOP + SMALL_ITEM_HEIGHT, this._menuBar.height);
		Assert.equals(PADDING_LEFT + SMALL_ITEM_WIDTH, this._menuBar.minWidth);
		Assert.equals(PADDING_TOP + SMALL_ITEM_HEIGHT, this._menuBar.minHeight);
		Assert.equals(Math.POSITIVE_INFINITY, this._menuBar.maxWidth);
		Assert.equals(Math.POSITIVE_INFINITY, this._menuBar.maxHeight);
	}
	...
	public function testAutoSizeWithMultipleChildren():Void {
		...
		this._menuBar.layout = layout;
		this._menuBar.validateNow();
		Assert.equals(PADDING_LEFT + 2.0 * SMALL_ITEM_WIDTH, this._menuBar.width);
		Assert.equals(PADDING_TOP + SMALL_ITEM_HEIGHT, this._menuBar.height);
		Assert.equals(PADDING_LEFT + 2.0 * SMALL_ITEM_WIDTH, this._menuBar.minWidth);
		Assert.equals(PADDING_TOP + SMALL_ITEM_HEIGHT, this._menuBar.minHeight);
		Assert.equals(Math.POSITIVE_INFINITY, this._menuBar.maxWidth);
		Assert.equals(Math.POSITIVE_INFINITY, this._menuBar.maxHeight);
	}
	...
	public function testAutoSizeWithWideChildAndBackgroundSkin():Void {
		...
		this.addComplexBackgroundSkin();
		this._menuBar.validateNow();
		Assert.equals(LARGE_ITEM_WIDTH, this._menuBar.width);
		Assert.equals(SMALL_BACKGROUND_HEIGHT, this._menuBar.height);
		Assert.equals(LARGE_ITEM_WIDTH, this._menuBar.minWidth);
		Assert.equals(SMALL_BACKGROUND_HEIGHT, this._menuBar.minHeight);
		Assert.equals(Math.POSITIVE_INFINITY, this._menuBar.maxWidth);
		Assert.equals(Math.POSITIVE_INFINITY, this._menuBar.maxHeight);
	}
	...
	public function testAutoSizeWithTallChildAndBackgroundSkin():Void {
		...
		this._menuBar.validateNow();
		Assert.equals(SMALL_BACKGROUND_WIDTH, this._menuBar.width);
		Assert.equals(LARGE_ITEM_HEIGHT, this._menuBar.height);
		Assert.equals(SMALL_BACKGROUND_WIDTH, this._menuBar.minWidth);
		Assert.equals(LARGE_ITEM_HEIGHT, this._menuBar.minHeight);
		Assert.equals(Math.POSITIVE_INFINITY, this._menuBar.maxWidth);
		Assert.equals(Math.POSITIVE_INFINITY, this._menuBar.maxHeight);
	}
}

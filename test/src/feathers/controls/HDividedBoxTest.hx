...
@:keep
class HDividedBoxTest extends Test {
	...
	private var _dividedBox:HDividedBox;
	...
	public function testThreeChildrenIncludeInLayoutFalseOnFirstChild():Void {
		...
		this._dividedBox.addChild(this._control3);
		this._dividedBox.validateNow();
		Assert.equals(CHILD2_WIDTH + DIVIDER_SIZE + CHILD3_WIDTH, this._dividedBox.width);
		Assert.equals(Math.max(CHILD2_HEIGHT, CHILD3_HEIGHT), this._dividedBox.height);
	}
	...
	public function testThreeChildrenIncludeInLayoutFalseOnMiddleChild():Void {
		...
		this._dividedBox.addChild(this._control3);
		this._dividedBox.validateNow();
		Assert.equals(CHILD1_WIDTH + DIVIDER_SIZE + CHILD3_WIDTH, this._dividedBox.width);
		Assert.equals(Math.max(CHILD1_HEIGHT, CHILD3_HEIGHT), this._dividedBox.height);
	}
	...
	public function testThreeChildrenIncludeInLayoutFalseOnFirstChildAfterValidate():Void {
		...
		this._dividedBox.validateNow();
		Assert.equals(CHILD2_WIDTH + DIVIDER_SIZE + CHILD3_WIDTH, this._dividedBox.width);
		// height will not get reset on other children after removing the

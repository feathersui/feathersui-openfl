...
@:keep
class VectorCollectionTest extends Test {
	private static final TEXT_FILTER_ME = "__FILTER_ME__";

	private var _collection:VectorCollection<MockItem>;
	...
	public function setup():Void {
		...
		this._collection = new VectorCollection(Vector.ofArray([this._a, this._b, this._c, this._d]));
	}
	...
	public function testRemoveAllWithEmptyCollection():Void {
		this._collection = new VectorCollection();
		var changeEvent = false;
		this._collection.addEventListener(Event.CHANGE, function(event:Event):Void {
			changeEvent = true;
		});
	...
	public function testResetVector():Void {
		...
		this._collection.vector = newVector;
		...
		Assert.equals(newVector.length, this._collection.length, "Collection length must change after resetting collection with data of new size");
	}

	public function testResetVectorToNull():Void {
		this._collection.vector = null;
		Assert.notNull(this._collection.vector, "Setting collection source to null should replace with an empty value.");
	...
	public function testFilterFunction():Void {
		Assert.equals(this._collection.vector.length, this._collection.length, "Collection length must match source length if unfiltered");
		this._collection.filterFunction = filterFunction;
		Assert.notEquals(this._collection.vector.length, this._collection.length, "Collection length must not match source length if items are filtered");
		Assert.equals(this._collection.vector.length - 2, this._collection.length, "Collection length must account for filterFunction");
	...
	public function testSetFilterFunctionToNull():Void {
		...
		Assert.equals(this._collection.vector.length, this._collection.length,
	...
	public function testSortCompareFunction():Void {
		...
		Assert.equals(this._collection.vector.length, this._collection.length, "Collection length must not change if sorted");

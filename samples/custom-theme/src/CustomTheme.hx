...
class CustomTheme extends ClassVariantTheme {
	...
	private function setButtonStyles(button:Button):Void {
		...
		backgroundSkin.setBorderForState(ButtonState.DOWN, SolidColor(1.0, 0xcc0000));
		backgroundSkin.fill = SolidColor(0xffffff);
		backgroundSkin.setFillForState(ButtonState.DOWN, SolidColor(0xffeeee));
	...
	private function setFancyButtonStyles(button:Button):Void {
		...
		backgroundSkin.setBorderForState(ButtonState.DOWN, Gradient(2, LINEAR, [0xff0000, 0xcc0000], [1.0, 1.0], [0, 255], 90 * Math.PI / 180));
		backgroundSkin.fill = Gradient(LINEAR, [0xff9999, 0xff0000], [1.0, 1.0], [0, 255], 90 * Math.PI / 180);
		backgroundSkin.setFillForState(ButtonState.DOWN, Gradient(LINEAR, [0xff9999, 0xff0000], [1.0, 1.0], [0, 255], 270 * Math.PI / 180));
		button.backgroundSkin = backgroundSkin;
